...
The Redtail image is currently not on the Docker Hub so you will have to build it first. Redtail project uses [NVIDIA TensorRT 4.0](https://developer.nvidia.com/tensorrt) library which has to be downloaded first. Make sure to download **TensorRT 4.0.1.6 for Ubuntu 16.04 and CUDA 9.0** tar package.

Next, go to `redtail/tools/simulation/docker` and run the build script:
```
./build_redtail_image.sh <path to TensorRT-4.0.1.6 tar file>
```
